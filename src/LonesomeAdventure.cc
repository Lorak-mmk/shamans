...
uint64_t LonesomeAdventure::packEggs(const std::vector<Egg>& eggs,
                                     BottomlessBag& bag) {
  ...
  for (size_t i = 1; i <= eggs.size(); i++) {
    ...
    for (size_t w = 0; w <= bag.getCapacity(); w++) {
      if (egg.getSize() > w) {
        ...
        tab[i][w] = std::max(tab[i - 1][w],
                             tab[i - 1][w - egg.getSize()] + egg.getWeight());
      }
    }
  }
...
Crystal LonesomeAdventure::selectBestCrystal(
