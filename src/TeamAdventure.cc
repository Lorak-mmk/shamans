...
template<typename T>
std::vector<std::pair<size_t, size_t>> divideArray(T *array, size_t N, size_t threads) {
	std::vector<std::pair<size_t, size_t>> result;
	// TODO: proper algo (align to page etc)
	size_t part_size = N / threads;
	size_t remainder = N - part_size * threads;
	for(size_t start = 0, end = part_size; start < N; start = end, end = start + part_size) {
		if (remainder) {
			end++;
			remainder--;
		}
...
uint64_t TeamAdventure::packEggs(const std::vector<Egg> &eggs, BottomlessBag &bag) {
	uint64_t **tab = alloc2DArray<uint64_t>(eggs.size() + 1, bag.getCapacity() + 1);
	std::fill(tab[0], tab[0] + bag.getCapacity() + 1, 0);

	auto f = [](auto *tab_i, auto *tab_ip, size_t i, size_t a, size_t b, Egg egg) {
		for(size_t w = a; w < b; w++) {
			if (egg.getSize() > w) {
				tab_i[w] = tab_ip[w];
			} else {
				tab_i[w] = std::max(tab_ip[w], tab_ip[w - egg.getSize()] + egg.getWeight());
			}
		}
	};
	...
	for(size_t i = 1; i <= eggs.size(); i++) {
		auto &egg = eggs[i - 1];
		for(auto p : idxs) {
			auto future = councilOfShamans.enqueue(f, tab[i], tab[i - 1], i, p.first, p.second, egg);
...
class mergeInfo {
public:
	static mergeInfo *createTree(std::vector<GrainOfSand> &arr, std::vector<std::pair<size_t, size_t>> &idxs, GrainOfSand *tmp) {
	...
	std::atomic<short> childrenReady{0};
...
void TeamAdventure::arrangeSand(std::vector<GrainOfSand> &grains) {
	...
	std::function<void(mergeInfo *)> f2 = [this, &f2, &cv](mergeInfo *node) {
		if (!node->right->unused) {
			merge(node->left->begin, std::distance(node->left->begin, node->left->end),
	...
	auto f1 = [this, &f2, &cv](auto begin, auto end, mergeInfo *node) {
		mergeSortSingle<std::vector<GrainOfSand>::iterator>(begin, end, node->buffer);
		node->ready = true;
		if (node->parent == nullptr) {
			cv.notify_one();
		} else if (++node->parent->childrenReady == 2) {
	...
	for(size_t i = 0; i < idxs.size(); i++) {
		auto &p = idxs[i];
		auto future = councilOfShamans.enqueue(f1, grains.begin() + p.first, grains.begin() + p.second, &tree[N + i]);
	}
...
Crystal TeamAdventure::selectBestCrystal(const std::vector<Crystal> &crystals) {
	...
	for(auto el : idxs) {
		auto future = councilOfShamans.enqueue(f, crystals.begin() + el.first, crystals.begin() + el.second);
